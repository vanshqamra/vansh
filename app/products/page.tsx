...
function SearchResults() {
  ...
  // Token-based search across combined fields
  ...
  const triggerSearch = (rawQuery: string) => {
    ...
    const rankemResults = (Array.isArray(rankemProducts) ? rankemProducts : []).flatMap((group: any) => {
      ...
      return variants
        .map((variant: any) => {
          // Name: Description or Unnamed: 1 fallback
          const name =
            str(variant["Description"]) ||
            str(variant["Unnamed: 1"]) ||
            str(group?.title)

          // Code: various "Cat No" fields, Code, OR special "Baker Analyzed..." header
          const code =
            str(variant["Cat No"]) ||
            str(variant["Cat No."]) ||
            ...
            str(variant["Code"]) ||
            str(variant["Baker Analyzed ACS\nReagent\n(PVC)"]) // ensure exact key with closing ')'

          // Pack size across multiple odd headers
          const packRaw =
            variant["Pack\nSize"] ??
            variant["Pack Size"] ??
            variant["Packing"] ??
          ...
          // Price across known headers (POR-safe)
          const price =
            variant["List Price\n2025(INR)"] ??
            variant["Price"] ??
            variant["Unnamed: 5"] ??
          ...
          // Skip rows that have neither name nor code
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedResults.map((product) => {
                ...
                const showBadgeB = !!badgeB && badgeB.toLowerCase() !== badgeA.toLowerCase()
